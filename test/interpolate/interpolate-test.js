...
suite.addBatch({
  "interpolate": {
    ...
    "when b is a number": {
      "interpolates numbers": function(d3) {
        assert.strictEqual(d3.interpolate(2, 12)(0.25), 4.5);
      },
      "coerces a to a number": function(d3) {
        assert.strictEqual(d3.interpolate("", 1)(0.5), 0.5);
        assert.strictEqual(d3.interpolate("2", 12)(0.25), 4.5);
        assert.strictEqual(d3.interpolate([2], 12)(0.25), 4.5);
      }
    },
    ...
    "when b is a color object": {
      "interpolates RGB values and returns a hexadecimal string": function(d3) {
        assert.strictEqual(d3.interpolate(d3.rgb(255, 0, 0), d3.rgb(0, 128, 0))(0.4), "#993300");
      },
      "interpolates d3.hsl in RGB": function(d3) {
        assert.strictEqual(d3.interpolate(d3.hsl("red"), d3.hsl("green"))(0.4), "#993300");
      },
      "interpolates d3.lab in RGB": function(d3) {
        assert.strictEqual(d3.interpolate(d3.lab("red"), d3.lab("green"))(0.4), "#993300");
      },
      "interpolates d3.hcl in RGB": function(d3) {
        assert.strictEqual(d3.interpolate(d3.hcl("red"), d3.hcl("green"))(0.4), "#993300");
      },
      "coerces a to a color": function(d3) {
        assert.strictEqual(d3.interpolate({toString: function() { return "red"; }}, "green")(0.4), "#993300");
      }
    ...
    "when b is a string": {
      "interpolates matching numbers in both strings": function(d3) {
        assert.strictEqual(d3.interpolate(" 10/20 30", "50/10 100 ")(0.4), "26/16 58 ");
      },
      "if b is coercible to a number, still returns a string": function(d3) {
        assert.strictEqual(d3.interpolate("1.", "2.")(0.5), "1.5");
        assert.strictEqual(d3.interpolate("1e+3", "1e+4")(0.5), "5500");
      },
      "preserves non-numbers in string b": function(d3) {
        assert.strictEqual(d3.interpolate(" 10/20 30", "50/10 foo ")(0.4), "26/16 foo ");
      },
      "preserves non-matching numbers in string b": function(d3) {
        assert.strictEqual(d3.interpolate(" 10/20 bar", "50/10 100 ")(0.4), "26/16 100 ");
      },
      "preserves equal-value numbers in both strings": function(d3) {
        assert.strictEqual(d3.interpolate(" 10/20 100 20", "50/10 100, 20 ")(0.4), "26/16 100, 20 ");
      },
      "coerces a to a string": function(d3) {
        assert.strictEqual(d3.interpolate({toString: function() { return "1."; }}, "2.")(0.5), "1.5");
      }
    },

    "when b is an array": {
      "interpolates each element in b": function(d3) {
        assert.strictEqual(JSON.stringify(d3.interpolate([2, 4], [12, 24])(0.25)), "[4.5,9]");
      },
      "interpolates arrays, even when both a and b are coercible to numbers": function(d3) {
        assert.strictEqual(JSON.stringify(d3.interpolate([2], [12])(0.25)), "[4.5]");
        assert.strictEqual(JSON.stringify(d3.interpolate([[2]], [[12]])(0.25)), "[[4.5]]");
      },
    ...
    "when b is an object": {
      "interpolates each property in b": function(d3) {
        assert.deepEqual(d3.interpolate({foo: 2, bar: 4}, {foo: 12, bar: 24})(0.25), {foo: 4.5, bar: 9});
      },
      "interpolates numbers if b is coercible to a number (!isNaN(+b))": function(d3) {
        assert.strictEqual(d3.interpolate(new Number(2), new Number(12))(0.25), 4.5);
        assert.strictEqual(d3.interpolate(new Date(2012, 0, 1), new Date(2013, 0, 1))(0.5), +new Date(2012, 6, 2, 1));
        assert.strictEqual(d3.interpolate(1, null)(0.4), 0.6); // +null = 0
        assert.isNaN(d3.interpolate("blue", null)(0.4));
      },
    ...
    "interpolates inherited properties of objects": function(d3) {
      ...
      assert.deepEqual(d3.interpolate(a, b)(0.5), {foo: 1});
    },
  ...
  "interpolators": {
    ...
    "can register a custom interpolator": function(d3) {
      ...
      try {
        assert.equal(d3.interpolate("one", "two")(-0.5), 0.5);
        assert.equal(d3.interpolate("one", "two")(0), 1);
        assert.equal(d3.interpolate("one", "two")(0.5), 1.5);
