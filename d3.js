!function() {
  ...
  var WATCH_ATTR = /^(x|y|width|height|transform)$/;
  ...
  d3_selectionPrototype.html = function(value) {
    ...
    return arguments.length < 1 ? (fo && this.node() || this.node().childNodes[0]).innerHTML : this.each(typeof value === "function" ? function() {
      var v = value.apply(this, arguments);
      if (v == undefined) {
        return;
      }
  ...
  d3_selectionPrototype.filter = function(filter) {
    ...
    for (var j = 0, m = this.length; j < m; j++) {
      ...
      for (var i = 0, n = group.length; i < n; i++) {
        if ((node = group[i]) && filter.call(node, node[dataProperty], i, j)) {
          subgroup.push(node);
        }
      }
    }
    ...
    function dragstart(id, position, subject, move, end) {
      return function() {
        ...
        function moved() {
          ...
          if (!position1) return;
          if (!dragEventFired && max(abs(position1[0] - position0[0]), abs(position1[1] - position0[1])) > 5) {
            dispatch({
              type: "dragstart"
            });
            dragEventFired = true;
  ...
  d3.behavior.zoom = function() {
    var view = {
      ...
      y: 0,
      k: 1
    }, translate0, center0, center, size = [ 960, 500 ], scaleExtent = d3_behavior_zoomInfinity, duration = 250, zooming = 0, mousedown = "mousedown.zoom", mousemove = "mousemove.zoom", mouseup = "mouseup.zoom", rectSelect = [ 0, 0, 0, 0 ], mousewheelTimer, touchstart = "touchstart.zoom", touchtime, event = d3_eventDispatch(zoom, "zoomstart", "zoom", "zoomend", "rectSelect", "rectFinish"), x0, x1, y0, y1;
    ...
    function dispatchRS(event, type) {
      d3.event.preventDefault();
      event({
        type: type || "rectSelect",
        select: rectSelect
      });
    }
    function mousedowned() {
      var that = this, dispatch = event.of(that, arguments), dragged = 0, subject = d3.select(d3_window(that)).on(mousemove, moved).on(mouseup, ended), location0 = location(d3.mouse(that)), rectSelectFlag = d3.event.metaKey || d3.event.ctrlKey, dragRestore = d3_event_dragSuppress(that);
  ...
  function d3_transition_tween(groups, name, value, tween) {
    ...
    return d3_selection_each(groups, typeof value === "function" ? function(node, i, j) {
      node[ns][id].tween.set(name, tween(value.call(node, node[dataProperty], i, j)));
  ...
  d3_transitionPrototype.delay = function(value) {
    ...
    return d3_selection_each(this, typeof value === "function" ? function(node, i, j) {
      node[ns][id].delay = +value.call(node, node[dataProperty], i, j);
  ...
  d3_transitionPrototype.duration = function(value) {
    ...
    return d3_selection_each(this, typeof value === "function" ? function(node, i, j) {
      node[ns][id].duration = Math.max(1, value.call(node, node[dataProperty], i, j));
