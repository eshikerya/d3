!function() {
  ...
  d3.behavior.zoom = function() {
    var view = {
      ...
      y: 0,
      k: 1
    }, translate0, center, size = [ 960, 500 ], scaleExtent = d3_behavior_zoomInfinity, mousedown = "mousedown.zoom", mousemove = "mousemove.zoom", mouseup = "mouseup.zoom", rectSelect = [ 0, 0, 0, 0 ], mousewheelTimer, touchstart = "touchstart.zoom", touchtime, event = d3_eventDispatch(zoom, "zoomstart", "zoom", "zoomend", "rectSelect", "rectFinish"), x0, x1, y0, y1;
    function zoom(g) {
    ...
    function mousedowned() {
      var that = this, target = d3.event.target, dispatch = event.of(that, arguments), dragged = 0, subject = d3.select(d3_window).on(mousemove, moved).on(mouseup, ended), location0 = location(d3.mouse(that)), rectSelectFlag = d3.event.metaKey || d3.event.ctrlKey, dragRestore = d3_event_dragSuppress();
  ...
  d3_transitionPrototype.duration = function(value) {
    ...
    return d3_selection_each(this, typeof value === "function" ? function(node, i, j) {
      node.__transition__[id].duration = Math.max(1, value.call(node, node[dataProperty], i, j));
  ...
  function d3_transitionNode(node, i, id, inherit) {
    ...
    if (!transition) {
      ...
      d3.timer(function(elapsed) {
        var d = node[dataProperty], ease = transition.ease, delay = transition.delay, duration = transition.duration, timer = d3_timer_active, tweened = [];
        timer.t = delay + time;
